...
chrs <- c(1:20, "X", "Y")
d_median <- read_tsv("data/mapped_read_counts.txt", col_types = "ccdd")
...
exons_rn6 <- read_tsv("data/exons_rn6.txt.gz", col_types = "cii",
                      col_names = c("chr", "start", "end")) %>%
  left_join(names_rn6, by = "chr") %>%
  filter(!is.na(chrom))
exons_rn7 <- read_tsv("data/exons_rn7.txt.gz", col_types = "cii",
                      col_names = c("chr", "start", "end")) %>%
  left_join(names_rn7, by = "chr") %>%
  filter(!is.na(chrom))

exons_len_rn6 <- exons_rn6 %>%
  rename(chr_rn6 = chrom) %>%
  group_by(chr_rn6) %>%
  summarise(exons_len_rn6 = sum(abs(end - start) + 1))
exons_len_rn7 <- exons_rn7 %>%
  rename(chr_rn7 = chrom) %>%
  group_by(chr_rn7) %>%
  summarise(exons_len_rn7 = sum(abs(end - start) + 1))
...
comp <- seg_total %>%
  left_join(d_median, by = c("chr_rn6", "chr_rn7")) %>%
  left_join(chrom_len_rn6, by = "chr_rn6") %>%
  left_join(chrom_len_rn7, by = "chr_rn7") %>%
  left_join(exons_len_rn6, by = "chr_rn6") %>%
  left_join(exons_len_rn7, by = "chr_rn7") %>%
  mutate(norm_genome_remap = (bases_remapped * 1e12) / (len_rn6 * len_rn7),
